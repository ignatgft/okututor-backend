...
| 21 | PgSchedule | availability CRUD | GET/POST/DELETE | `/availability`, `/availability/{id}` | AvailabilityRequest `{weekday,start_time,end_time,timezone?(IANA, default UTC)}` | `[AvailabilityResponse{id,weekday,start_time,end_time,timezone}]`; неверная timezone → 422; пересечение слотов того же дня → 422 | TUTOR | AvailabilityService | AvailabilitySlot | availability_slots | AvailabilityServiceTest |
| 21a | — | common availability slots | GET | `/availability/common-slots?tutor_id&student_id&date(yyyy-MM-dd)` | — | `[CommonSlot{date,start_time,end_time,start_local,end_local}]` — пересечение доступности двух пользователей на дату в UTC | authenticated | AvailabilityService | AvailabilitySlot | availability_slots | AvailabilityServiceTest |
...
| 36b | — | my applications (v2) | GET | `/applications/me` | — | `[EnrollmentResponse]` | STUDENT | EnrollmentService | Enrollment | enrollments | — |
| 36c | — | application timeline | GET | `/applications/{id}/timeline` | — | `[TimelineItemResponse{action,created_at,actor_id,actor_name,actor_role,message,old_value,new_value}]` | участник / ADMIN | ApplicationWorkflowService | AuditLog | audit_logs | ApplicationWorkflowServiceTest |
| 36d | — | request info | POST | `/applications/{id}/request-info` | `{request}` | EnrollmentResponse (PENDING→NEEDS_INFO, аудит APPLICATION_REQUEST_INFO, уведомление APPLICATION_NEEDS_INFO) | TUTOR owner | ApplicationWorkflowService | Enrollment, AuditLog | enrollments, audit_logs | ApplicationWorkflowServiceTest |
| 36e | — | submit info | POST | `/applications/{id}/submit-info` | `{message}` | EnrollmentResponse (NEEDS_INFO→PENDING, аудит APPLICATION_INFO_SUBMITTED, уведомление APPLICATION_INFO_SUBMITTED) | STUDENT owner | ApplicationWorkflowService | Enrollment, AuditLog | enrollments, audit_logs | ApplicationWorkflowServiceTest |
| 36f | — | propose schedule | POST | `/schedule/applications/{applicationId}/propose` | ProposeRequest `{timezone?,format?,start_date(yyyy-MM-dd),end_date,duration_minutes,slots:[{weekday:MONDAY..SUNDAY,start_time,end_time}],message?,location_type?,location_address?,location_details?}` | ScheduleProposalResponse `{id,schedule_id,application_id,created_by,created_by_name,created_by_role,status:"PENDING",timezone,start_date,end_date,duration_minutes,message,slots,created_at}`; при уже висящем PENDING → 409 `SCHEDULE_NOT_AVAILABLE`; слоты ≥ duration_minutes; для `format=OFFLINE` обязателен `location_type` (TUTOR_PLACE/STUDENT_PLACE/CENTER/OTHER) → иначе 422 | TUTOR owner (курс) | ScheduleService | Schedule, ScheduleProposal | schedules, schedule_proposals | ScheduleServiceTest |
| 36g | — | proposals list / detail | GET | `/schedule/applications/{id}/proposals` , `/schedule/proposals/{id}` | — | `[ScheduleProposalResponse]` / ScheduleProposalResponse (история всех предложений, устаревшие = SUPERSEDED) | участник | ScheduleService | ScheduleProposal | schedule_proposals | — |
| 36h | — | accept proposal | POST | `/schedule/proposals/{id}/accept` | — | AcceptResponse `{schedule:ScheduleResponse,created_count,conflicted_dates,booking_ids}`; атомарно: proposal→ACCEPTED (остальные PENDING→SUPERSEDED), schedule→CONFIRMED, enrollment→SCHEDULED, материализуются Booking+Lesson на `[start_date,end_date]` по слотам (конфликтные даты пропускаются); повторный accept идемпотентен; уведомления SCHEDULE_CONFIRMED обеим сторонам; аудит SCHEDULE_CONFIRMED + LESSONS_GENERATED | STUDENT owner | ScheduleService | ScheduleProposal, Schedule, Booking, Lesson | schedule_proposals, schedules, bookings, lessons | ScheduleServiceTest |
| 36i | — | reject proposal | POST | `/schedule/proposals/{id}/reject` | — | ScheduleProposalResponse (proposal→REJECTED, enrollment→SCHEDULE_PENDING, уведомление SCHEDULE_CHANGED тьютору) | STUDENT owner | ScheduleService | ScheduleProposal, Enrollment | schedule_proposals, enrollments | ScheduleServiceTest |
| 36j | — | counter-proposal | POST | `/schedule/proposals/{id}/counter` | ProposeRequest | ScheduleProposalResponse (старое→SUPERSEDED, новое PENDING, enrollment→SCHEDULE_PROPOSED, уведомление SCHEDULE_PROPOSED тьютору) | STUDENT owner | ScheduleService | ScheduleProposal, Schedule | schedule_proposals, schedules | ScheduleServiceTest |
| 36k | — | my schedules | GET | `/schedule/me` | — | `[ScheduleResponse{id,application_id,course_id,course_title,student_id,student_name,tutor_id,tutor_name,format,location_type,location_address,location_details,start_date,end_date,timezone,frequency,duration_minutes,status,slots,booked_count,conflicted_count,created_at,updated_at}]` | STUDENT (свои) / TUTOR (свои) | ScheduleService | Schedule | schedules | ScheduleServiceTest |
| 36l | — | schedule detail / lessons | GET | `/schedule/{id}` , `/schedule/{id}/lessons` | — | ScheduleResponse / `[LessonResponse]` | участник schedule | ScheduleService | Schedule, Lesson | schedules, lessons | — |
| 36m | — | free slots for application | GET | `/schedule/applications/{applicationId}/available-slots?from&to&timezone` | — | `[AvailableSlotResponse{date,start_time,end_time,source:"PREFERENCE"\|"AVAILABILITY"}]` — пересечение предпочтений студента и доступности тьютора минус занятость участников | участник | ScheduleService | AvailabilitySlot, Booking, Lesson | availability_slots, bookings, lessons | ScheduleServiceTest |
...
| 44 | PgLessons/Schedule | lessons list | GET | `/lessons` | — | Page\<LessonResponse `{id,title,counterpart,start_at,end_at,status,joinable,booking_id,schedule_id,cancel_reason,location_type,location_address,location_details}`\> | authenticated | LessonService | Lesson | lessons | — |
| 45 | Tutor lessons | create/start/complete/cancel/reschedule | POST | `/lessons`, `/lessons/{id}/start|complete|cancel|reschedule` | LessonCreateRequest; cancel — optional `{message}`; reschedule — `{start_at,end_at?}` (ISO-8601 UTC, длительность из {30,45,60,90,120}); create — `{student_id,course_id?,title?,start_at,format?,location_type?,location_address?,location_details?}` | LessonResponse `{id,title,counterpart,start_at,end_at,status,joinable,booking_id,schedule_id,cancel_reason,location_type,location_address,location_details}`; reschedule перепроверяет занятость участников → 409 `LESSON_CONFLICT`, зеркалит Booking, аудит LESSON_RESCHEDULED; create `format=OFFLINE` требует `location_type` → 422 | TUTOR/ADMIN (участник для start/complete/cancel/reschedule) | LessonService | Lesson, Booking | lessons, bookings | LessonStateMachineTest, LessonServiceTest |
...
| 46b | PgMessages | upload attachment | POST multipart | `/messages/attachments` | form field **file** (jpg/png/webp/gif/pdf/txt/doc/docx, ≤10 MB) | MessageAttachmentRef `{media_id,url,thumbnail_url,filename,content_type,size,kind:"IMAGE"\|"FILE"}` | authenticated | MediaService | MessageAttachment, MediaObject | message_attachments, media_objects | — |
| 47 | PgMessages thread | conversation messages | GET | `/messages/conversations/{id}` | — | `[MessageResponse {id,sender_id,sender_name,body,attachment:{media_id,url,thumbnail_url,filename,content_type,size,kind},created_at,read_at}]`; attachment=null для текстовых сообщений | участник | MessagingService | Message | messages | — |
| 48 | PgMessages send | send | POST | `/messages` | JSON `{conversation_id,body,attachment_media_id?}`; **или** multipart (conversation_id, body, file) | MessageResponse (с attachment при наличии) | участник DIRECT | MessagingService | Message, Notification | messages, message_attachments | MessageAttachmentIT |
...
| 53 | PgSupportTicket | messages | GET/POST | `/support/tickets/{id}/messages` | JSON `{body}`; **или** multipart (body, file ≤10 MB) | `[SupportMessageResponse {id:"msg-…",ticket_id,sender_id,sender_name,sender_role,body,created_at,type:"USER_VISIBLE",attachments:[{media_id,url,thumbnail_url,filename,content_type,size,kind}],client_status:"SENT"}]` | author / admin | SupportMessageService | SupportTicketMessage | support_ticket_messages, message_attachments | — |
...
14a. **Домен «заявка → расписание → занятия»** (V25–V29). Error-коды: `APPLICATION_NOT_FOUND`
    (404), `INVALID_APPLICATION_STATE` (409, нелегальный переход статуса — единственная точка
    валидации — `Enrollment.transitionTo`), `SCHEDULE_NOT_AVAILABLE` (409, уже есть PENDING
    предложение / в расписании нет слотов), `SCHEDULE_CONFLICT` (409), `LESSON_CONFLICT`
    (409, reschedule пересекается с занятостью участника), `INVALID_TIMEZONE`, `INVALID_DATE`
    (422), `NOT_APPLICATION_OWNER` (403), `NOT_COURSE_OWNER` (403). NotificationType:
    `APPLICATION_NEEDS_INFO`, `APPLICATION_INFO_SUBMITTED`, `SCHEDULE_PROPOSED`,
    `SCHEDULE_CONFIRMED`, `SCHEDULE_CHANGED`, `LESSON_CREATED`, `LESSON_RESCHEDULED`,
    `LESSON_CANCELLED`, `LESSON_STARTED`, `LESSON_COMPLETED`, `LESSON_REMINDER`. Уведомления
    расписания/занятий несут `entity_type` (`APPLICATION`/`LESSON`) + `entity_id`, а напоминания —
    ещё и `payload.window` (`24h`/`15m`) для дедупа.
14b. **Таймлайн заявки** = история из `audit_logs` (actions `APPLICATION_*`, `SCHEDULE_*`,
    `LESSONS_GENERATED`) с `old_value/new_value`; аудит пишется синхронно в той же транзакции,
    что и переход статуса.
